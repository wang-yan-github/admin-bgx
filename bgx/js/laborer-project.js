var loadAddress = 'https://bgx.lingser.cn/getworkerprojectlist?token=' + customSession.data.token;
var modalAuditFormAddress = 'https://bgx.lingser.cn/changeworkerproject?token=' + customSession.data.token;
var modalDetailFormAddress = 'https://bgx.lingser.cn/getworkerprojectdetail?token=' + customSession.data.token;
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    if(row.wo_status == 311){
                        var res = '&nbsp;&nbsp;&nbsp;&nbsp;<button class="btn btn-outline btn-success btn-xs" onclick="getAudit(\'' + row.wo_id + '\')">项目完工</button>';
                    }
...
$(function () {
    $.ajax({
        ...
        success: function (data) {
            if(data.code == 200){
                // 初始查询下拉信息
                $('#w_status').append(new Option('',''));
                for(var i =0; i < data.data.types.length; i ++){
                    $('#w_status').append(new Option(data.data.types[i][1],data.data.types[i][0]));
                }
            }else{
...
// 获取审核
function getAudit(wo_id){
    $('#modal-audit-form input[name=wo_id]').val(wo_id);
    $('#modal-audit-form [name=re_content]').val('');
    $.ajax({
        type:'post',
