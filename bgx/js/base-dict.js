var loadAddress = costomUrl + 'getdatadictlist?token=' + customSession.data.token;
var modalAddFormAddress = costomUrl + 'adddatadict?token=' + customSession.data.token;
var modalEditFormAddress = costomUrl + 'editdatadict?token=' + customSession.data.token;
var deleteAddress = costomUrl + 'deletedatadict?token=' + customSession.data.token;
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    var res = '<button class="btn btn-outline btn-info btn-xs" onclick="getEdit(\'' + row.id + '\',\'' + row.t_code + '\',\'' + row.d_code + '\',\'' + row.d_name +'\')">修改</button>' +
        ...
        success: function (data) {
            if(data.code == 200){
                ...
                for(var i =0; i < data.data.types.length; i ++){
                    $('#code').append(new Option(data.data.types[i][1],data.data.types[i][0]));
                }
                // 初始新增下拉信息
                for(var i =0; i < data.data.types.length; i ++){
                    $('#formSave [name=t_code]').append(new Option(data.data.types[i][1],data.data.types[i][0]));
                }
                // 初始修改下拉信息
                for(var i =0; i < data.data.types.length; i ++){
                    $('#formEdit [name=t_code]').append(new Option(data.data.types[i][1],data.data.types[i][0]));
                }
            }else{
    ...
    $.ajax({
        ...
        success: function (data) {
            if(data.code == 200){
                ...
                $('#formSave [name="t_code"]').find("option").each(function (i) {
                    $(this).removeAttr("selected");
                });
                $('#formSave input[name=d_code]').val('');
                $('#formSave input[name=d_name]').val('');
                // 添加成功,重新刷新页面
...
// 获取修改信息
function getEdit(id,t_code,d_code,d_name) {
    $('#formEdit input[name="id"]').val('');
    $('#formEdit input[name="id"]').val(id);
    $('#formEdit [name=t_code]').find('option').each(function (i) {
        $(this).removeAttr('selected');
    });
    $('#formEdit [name="t_code"]').find('option[value="' + t_code + '"]').attr('selected',true);
    $('#formEdit input[name="d_code"]').val(d_code);
    $('#formEdit input[name="d_name"]').val('');
    $('#formEdit input[name="d_name"]').val(d_name);
