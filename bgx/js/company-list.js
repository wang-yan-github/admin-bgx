var loadAddress = 'http://bgx.lingser.cn/getcompanyist?token=' + customSession.data.token;
var modalAddFormAddress = 'http://bgx.lingser.cn/addcompany?token=' + customSession.data.token;
var modalDetaileFormAddress = 'http://bgx.lingser.cn/getcompanydetaile?token=' + customSession.data.token;
var modalEditFormAddress = 'http://bgx.lingser.cn/editcompany?token=' + customSession.data.token;
var deleteAddress = 'http://bgx.lingser.cn/deleteompany?token=' + customSession.data.token;
var uploadAddress = 'http://bgx.lingser.cn/bgxuploadimage';
var modalAuditFormAddress = 'http://bgx.lingser.cn/auditcompany?token=' + customSession.data.token;
...
$(function () {
    $('.table').bootstrapTable({
        ...
        columns: [
            ...
            {
                field: 'c_legal_person',
                title: '法人名称',
                align: 'center'
            },
            {
                field: 'c_regi_capital',
                title: '注册资金',
                align: 'center'
            },
            {
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    if(row.c_status == 101){
                        var res = '<button class="btn btn-outline btn-primary btn-xs" onclick="getDetaileById(\'' + row.c_id + '\')">详情</button>' +
                            '&nbsp;&nbsp;&nbsp;&nbsp;<button class="btn btn-outline btn-info btn-xs" onclick="getEdit(\'' + row.c_id +'\')">修改</button>' +
                            // '&nbsp;&nbsp;&nbsp;&nbsp;<button class="btn btn-outline btn-danger btn-xs" onclick="deleteById(\'' + row.c_id + '\')">删除</button>' +
                            '&nbsp;&nbsp;&nbsp;&nbsp;<button class="btn btn-outline btn-success btn-xs" onclick="getAudit(\'' + row.c_id + '\')">审核</button>';
...
$(document).on('click', '#add', function(){
    ...
    $('#formSave [name=c_business_license_file]').attr('disabled',false);
    $('#formSave [name=c_safety_frod_permit_file]').attr('disabled',false);
    $('#formSave [name=c_account_open_cert_url_file]').attr('disabled',false);
    $('#formEdit [name=c_business_license_url]').attr('disabled',false);
    $('#formEdit [name=c_safety_frod_permit_file]').attr('disabled',false);
    $('#formEdit [name=c_account_open_cert_file]').attr('disabled',false);
    ...
    $.ajax({
        ...
        success: function (data) {
            if(data.code == 200){
                ...
                $('#formSave input[name=i_code]').val('');
                $('#formSave input[name=c_name]').val('');
                $('#formSave [name="c_province"]').find("option").each(function (i) {
                    $(this).removeAttr("selected");
                });
                $('#formSave [name="c_city"]').find("option").each(function (i) {
                    $(this).removeAttr("selected");
                });
                $('#formSave [name="c_province"] > option:first').attr('selected',true);
                $('#formSave [name="c_city"] > option:first').attr('selected',true);
                $('#formSave input[name=c_addr]').val('');
                $('#formSave input[name=c_legal_person]').val('');
                $('#formSave input[name=c_regi_capital]').val('');
                $('#formSave input[name=c_contact_name]').val('');
                $('#formSave input[name=c_contact_tel]').val('');
                $('#formSave input[name=c_id_card]').val('');
                $('#formSave input[name=c_unif_soci_info]').val('');
                $('#formSave input[name=c_business_license]').val('');
                $('#formSave [name=c_business_license_url]').attr('src','');
                $('#formSave input[name=c_safety_frod_permit]').val('');
                $('#formSave [name=c_safety_frod_permit_url]').attr('src','');
                $('#formSave input[name=c_business_license]').val('');
...
function getEdit(id) {
    $.ajax({
        ...
        type: "POST",// 方法类型
        dataType: "json",// 服务器返回的数据类型
        url: modalDetaileFormAddress,// url
        data: {'c_id':id},
        success: function (data) {
            if(data.code == 200){
                // 页面显示数据
                $('#formEdit input').val('');
                $('#formEdit input[name=c_id]').val(data.data.c_id);
                $('#formEdit input[name=u_id]').val(data.data.u_id);
                $('#formEdit input[name=i_code]').val(data.data.i_code);
                $('#formEdit input[name=c_name]').val(data.data.c_name);
                $('#formEdit [name="c_province"]').find("option").each(function (i) {
                    $(this).removeAttr("selected");
                });
                $('#formEdit [name="c_province"] > option:first').val(data.data.c_province).html(data.data.c_province);
                $('#formEdit [name="c_province"]').find('option[value=\''+ data.data.c_province +'\']').attr('selected',true);
                $('#formEdit [name="c_city"]').find("option").each(function (i) {
                    $(this).removeAttr("selected");
                });
                $('#formEdit [name="c_city"] > option:first').val(data.data.c_city).html(data.data.c_city);
                $('#formEdit [name="c_city"]').find('option[value=\''+ data.data.c_city +'\']').attr('selected',true);
                $('#formEdit input[name=c_addr]').val(data.data.c_addr);
                $('#formEdit input[name=c_legal_person]').val(data.data.c_legal_person);
                $('#formEdit input[name=c_regi_capital]').val(data.data.c_regi_capital);
                $('#formEdit input[name=c_contact_name]').val(data.data.c_contact_name);
                $('#formEdit input[name=c_contact_tel]').val(data.data.c_contact_tel);
                $('#formEdit input[name=c_id_card]').val(data.data.c_id_card);
                $('#formEdit input[name=c_unif_soci_info]').val(data.data.c_unif_soci_info);
                $('#formEdit input[name=c_business_license]').val(data.data.c_business_license);
                $('#formEdit input[name=c_safety_frod_permit]').val(data.data.c_safety_frod_permit);
                $('#formEdit input[name=c_account_open_cert]').val(data.data.c_account_open_cert);
                $('#formEdit [name=c_business_license_url]').attr('src','');
                $('#formEdit [name=c_business_license_url]').attr('src',data.data.business_license_url);
                $('#formEdit [name=c_safety_frod_permit_url]').attr('src','');
                $('#formEdit [name=c_safety_frod_permit_url]').attr('src',data.data.safety_frod_permit_url);
                $('#formEdit [name=c_account_open_cert_url]').attr('src','');
                $('#formEdit [name=c_account_open_cert_url]').attr('src',data.data.account_open_cert_url);
...
function getDetaileById(id) {
    $.ajax({
        ...
        success: function (data) {
            if(data.code == 200){
                // 显示数据
                $('#formDetaile input[name=i_code]').val(data.data.i_code);
                $('#formDetaile input[name=c_name]').val(data.data.c_name);
                $('#formDetaile input[name=c_province]').val(data.data.c_province);
                $('#formDetaile input[name=c_city]').val(data.data.c_city);
                $('#formDetaile input[name=c_addr]').val(data.data.c_addr);
                $('#formDetaile input[name=c_legal_person]').val(data.data.c_legal_person);
                $('#formDetaile input[name=c_regi_capital]').val(data.data.c_regi_capital);
                $('#formDetaile input[name=c_contact_name]').val(data.data.c_contact_name);
                $('#formDetaile input[name=c_contact_tel]').val(data.data.c_contact_tel);
                $('#formDetaile input[name=c_id_card]').val(data.data.c_id_card);
                $('#formDetaile input[name=c_unif_soci_info]').val(data.data.c_unif_soci_info);
                $('#formDetaile [name=business_license_url]').attr('src',data.data.business_license_url);
                $('#formDetaile [name=safety_frod_permit_url]').attr('src',data.data.safety_frod_permit_url);
                $('#formDetaile [name=account_open_cert_url]').attr('src',data.data.account_open_cert_url);
                $('#modal-detaile-form').modal('show');
            }else{
                toastr.error(data.msg)
            }
...
function getAudit(id){
    $.ajax({
        ...
        success: function (data) {
            if(data.code == 200){
                // 显示数据
                $('#formAudit input[name=c_id]').val(data.data.c_id);
                $('#formAudit input[name=i_code]').val(data.data.i_code);
                $('#formAudit input[name=c_name]').val(data.data.c_name);
                $('#formAudit input[name=c_province]').val(data.data.c_province);
                $('#formAudit input[name=c_city]').val(data.data.c_city);
                $('#formAudit input[name=c_addr]').val(data.data.c_addr);
                $('#formAudit input[name=c_legal_person]').val(data.data.c_legal_person);
                $('#formAudit input[name=c_regi_capital]').val(data.data.c_regi_capital);
                $('#formAudit input[name=c_contact_name]').val(data.data.c_contact_name);
                $('#formAudit input[name=c_contact_tel]').val(data.data.c_contact_tel);
                $('#formAudit input[name=c_id_card]').val(data.data.c_id_card);
                $('#formAudit input[name=c_unif_soci_info]').val(data.data.c_unif_soci_info);
                $('#formAudit [name=business_license_url]').attr('src',data.data.business_license_url);
                $('#formAudit [name=safety_frod_permit_url]').attr('src',data.data.safety_frod_permit_url);
                $('#formAudit [name=account_open_cert_url]').attr('src',data.data.account_open_cert_url);
...
$(document).on('change', '#formUpload [name=image]', function(){
    if($('#formUpload [name=type]').val() == 'save_c_business_license_file'){
        $('#formSave [name=c_business_license_file]').attr('disabled',false);
        $.ajax({
            url: uploadAddress,
            type: 'POST',
    ...
    }else if($('#formUpload [name=type]').val() == 'save_c_safety_frod_permit_file'){
        $('#formSave [name=c_safety_frod_permit_file]').attr('disabled',false);
        $.ajax({
            url: uploadAddress,
            type: 'POST',
    ...
    }else if($('#formUpload [name=type]').val() == 'save_c_account_open_cert_file'){
        $('#formSave [name=c_account_open_cert_file]').attr('disabled',false);
        $.ajax({
            url: uploadAddress,
            type: 'POST',
            cache: false,
    ...
    }else if($('#formUpload [name=type]').val() == 'edit_c_business_license_file'){
        $('#formEdit [name=c_business_license_file]').attr('disabled',false);
        $.ajax({
            url: uploadAddress,
            type: 'POST',
            cache: false,
    ...
    }else if($('#formUpload [name=type]').val() == 'edit_c_safety_frod_permit_file'){
        $('#formEdit [name=c_safety_frod_permit_file]').attr('disabled',false);
        $.ajax({
            url: uploadAddress,
            type: 'POST',
    ...
    }else if($('#formUpload [name=type]').val() == 'edit_c_account_open_cert_file'){
        $('#formEdit [name=c_account_open_cert_file]').attr('disabled',false);
        $.ajax({
            url: uploadAddress,
            type: 'POST',
...
var save_s=["save_c_province","save_c_city","save_c_area"];//三个select的name
function _init_area_save(){  //初始化函数
    for(i=0;i<save_s.length-1;i++){
        document.getElementById(save_s[i]).onchange=new Function("change_save("+(i+1)+")");
    }
    change_save(0);
}
function change_save(v){
    var str="0";
    for(i=0;i<v;i++){
        str+=("_"+(document.getElementById(save_s[i]).selectedIndex-1));
    };
    var ss=document.getElementById(save_s[v]);
    with(ss){
        length = 0;
        options[0]=new Option(opt0[v],opt0[v]);
        if(v && document.getElementById(save_s[v-1]).selectedIndex>0 || !v){
            if(dsy.Exists(str)){
                ar = dsy.Items[str];
        ...
        if(++v<save_s.length){change_save(v);}
    }//End with
}

var edit_s=["edit_c_province","edit_c_city","edit_c_area"];//三个select的name
function _init_area_edit(){  //初始化函数
    for(i=0;i<edit_s.length-1;i++){
        document.getElementById(edit_s[i]).onchange=new Function("change_edit("+(i+1)+")");
    }
    change_edit(0);
}
function change_edit(v){
    var str="0";
    for(i=0;i<v;i++){
        str+=("_"+(document.getElementById(edit_s[i]).selectedIndex-1));
    };
    var ss=document.getElementById(edit_s[v]);
    with(ss){
        length = 0;
        options[0]=new Option(opt0[v],opt0[v]);
        if(v && document.getElementById(edit_s[v-1]).selectedIndex>0 || !v){
            if(dsy.Exists(str)){
                ar = dsy.Items[str];
                for(i=0;i<ar.length;i++){
                    options[length]=new Option(ar[i],ar[i]);
        ...
        if(++v<edit_s.length){change_edit(v);}
    }//End with
}