var loadAddress = 'https://bgx.lingser.cn/getprotylist?token=' + customSession.data.token;
var modalAddFormAddress = 'https://bgx.lingser.cn/addproty?token=' + customSession.data.token;
var modalEditFormAddress = 'https://bgx.lingser.cn/editproty?token=' + customSession.data.token;
var deleteAddress = 'https://bgx.lingser.cn/deleteproty?token=' + customSession.data.token;
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    var res = '<button class="btn btn-outline btn-info btn-xs" onclick="getEdit(\'' + row.id + '\',\'' + row.d_code + '\',\'' + row.name +'\')">修改</button>' +
    ...
    $.ajax({
        ...
        success: function (data) {
            if(data.code == 200){
                ...
                // 保存下拉列表
                for(var i = 0; i < data.data.types.length; i ++){
                    $('#formSave [name=d_code]').append(new Option(data.data.types[i].d_name,data.data.types[i].d_code));
                }

                // 修改下拉列表
                for(var i = 0; i < data.data.types.length; i ++){
                    $('#formEdit [name=d_code]').append(new Option(data.data.types[i].d_name,data.data.types[i].d_code));
                }
            }else{
...
// 获取修改信息
function getEdit(id,d_code,name) {
